...
echo "You should run this from directory where you have cloned the react-native-new-orientation repo"
...
pushd example
npm install github:joaodematejr/react-native-yz-vlcplayer
npx react-native link react-native-yz-vlcplayer
...
popd
echo "Copying react-native-yz-vlcplayer example files into refreshed example..."
cp -frv TEMP/* example/
